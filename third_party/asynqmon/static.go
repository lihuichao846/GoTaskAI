...
func (h *uiAssetsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Clean URL paths directly instead of converting them to filesystem paths.
	// On Windows, filepath.Abs("/admin/tasks") becomes something like "D:\\admin\\tasks",
	// which breaks the RootPath prefix check and triggers "unexpected path prefix".
